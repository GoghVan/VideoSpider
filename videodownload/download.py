        ...
        try:
            with closing(requests.get(self.video_url, stream=True)) as response:
                ...
                if (os.path.exists(file_f) and os.path.getsize(file_f) == content_size or flag):
                    ...
                    progress = ProgressBar(time.time(), TimeNow.get_time(), self.video_message.encode('utf8'),
                                           total=content_size, unit="KB", chunk_size=chunk_size,
            ...
            print ('videodownload.download.video_download(53): ' + str(e))
