...
def update_message(check, conn, web_url, video_message):
    ...
    if 'http://rr.tv/' in web_url or 'http://www.rr.tv/' in web_url:
        ...
        sql4 = "UPDATE renren " \
               "SET Video_Comment=%s " \
               "WHERE Video_Web=%s"
        # video_message[1] = video_message[1].encode('utf8')
        x = video_message[5]
        y = video_message[6]
        z = video_message[7]
        video_message[6] = x
    ...
    # 视频观看数
    cur = conn.cursor()
    if (point == 1 or point == 2 or point == 3 or point == 4) and check[0].encode('utf8') != str(video_message[6]):
        try:
    ...
    # 视频点赞数
    cur = conn.cursor()
    if (point == 1 or point == 2 or point == 3 or point == 4) and check[2].encode('utf8') != str(video_message[8]):
        try:
    ...
    # 视频评论数
    cur = conn.cursor()
    if (point == 1 or point == 2 or point == 3) and check[3].encode('utf8') != str(video_message[9]):
        try:
        ...
        mess = TimeNow.get_time() + " 视频 : [ " + video_message[1] + " ] 的Video_Comment : " \
            + check[1].encode('utf8') + ", 现在Video_Comment : " + str(video_message[9])
        print (mess)
        Log.log(mess)
        flag7 = 0
    else:
