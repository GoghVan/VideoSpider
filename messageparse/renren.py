...
class RenRen(object):
    ...
    def rr_comment(self, web_url):
        try:
            ...
            for page in range(1, int(math.ceil(comment_count / 10)) + 1):
                req_js = requests.post(api_url, data={'videoId': vid, 'page': page}, headers=self.headers).content
                req_py = json.loads(req_js)
                for Comment in range(0, 10):
                    ...
                    content1 = u'%s[ %s # %s # %s # %s ] ' \
                               % (content1, comment_message[1], comment_message[2], comment_message[3], comment_message[4])
            ...
            print ('messageparse.renren.rr_comments(54): ' + str(e))
